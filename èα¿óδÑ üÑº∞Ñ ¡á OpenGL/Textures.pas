...
uses
  Windows, GL, GLu, GLext, Graphics, Classes, JPEG, SysUtils;

function LoadTexture(Filename: String; var Texture: GLuint; LoadFromRes : Boolean): Boolean;
function GetFreeTex() : GLuint;        // b!
var
    freeTex : GLuint;
    extTextureComp : boolean;   // extensions
...
//  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); { all of the above can be used }
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR_MIPMAP_LINEAR); { all of the above can be used }

  intFormat := Format;
  if Format = GL_RGBA then begin
      if(extTextureComp) then intFormat := GL_COMPRESSED_RGBA_ARB;
//      N := 4;
  end
  else begin
      if(extTextureComp) then intFormat := GL_COMPRESSED_RGB_ARB;
//      N := 3;
  end;

//  glTexImage2D( GL_TEXTURE_2D, 0, intFormat, Width, Height,
//                0, Format, GL_UNSIGNED_BYTE, pData);
    gluBuild2DMipmaps(GL_TEXTURE_2D, intFormat, Width, Height, Format, GL_UNSIGNED_BYTE, pData);
...
begin
  if(not FileExists(Filename)) then begin
    MessageBox(0, PChar('Fichier non trouvé : '+Filename), 'K-Vern', 0); 
    result := false;
    exit;
  end;
